...
u = TrialFunction(V)
v = TestFunction(V)
f = Expression('50*exp(-2*(pow((x[0]-t),2) + pow((x[1]-b*H),2) + pow(x[2], 1/2))/(d_x*d_x + d_y*d_y + d_z*d_z))', 
                degree=1, b=b, H=H,d_x=d_x,d_y=d_y,d_z=d_z, t = 0)
...
conc_f = File ("ResultsDir3D/phi.pvd")
